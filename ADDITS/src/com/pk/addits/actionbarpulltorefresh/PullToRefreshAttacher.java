...
package com.pk.addits.actionbarpulltorefresh;
...
public class PullToRefreshAttacher implements View.OnTouchListener
{
	...
	private static final int DEFAULT_ANIM_HEADER_IN = R.anim.quick_in_from_up;
	private static final int DEFAULT_ANIM_HEADER_OUT = R.anim.quick_fade_out;
	...
	private static final WeakHashMap<Activity, PullToRefreshAttacher> ATTACHERS = new WeakHashMap<Activity, PullToRefreshAttacher>();
	...
	/**
	 * Get a PullToRefreshAttacher for this Activity. If there is already a PullToRefreshAttacher attached to the Activity, the existing one is returned, otherwise a new instance is created. This
	 * version of the method will use default configuration options for everything.
	 * 
	 * @param activity
	...
	/**
	 * Add a view which will be used to initiate refresh requests and a listener to be invoked when a refresh is started. This version of the method will try to find a handler for the view from the
	 * built-in view delegates.
	 * 
	 * @param view
	...
	/**
	 * Manually set this Attacher's refreshing state. The header will be displayed or hidden as requested.
	 * 
	 * @param refreshing
	...
	/**
	 * Allows the enable/disable of this PullToRefreshAttacher. If disabled when refreshing then the UI is automatically reset.
	 * 
	 * @param enabled
	...
	/**
	 * Call this when your refresh is complete and this view should reset itself (header view will be hidden).
	 * 
		...
		switch (event.getAction())
		{
			case MotionEvent.ACTION_MOVE:
			{
				...
				if (mIsBeingDragged)
				{
					...
					/**
					 * Check to see if the user is scrolling the right direction (down). We allow a small scroll up which is the check against negative touch slop.
					 */
					if (yDx >= -mTouchSlop)
					{
						onPull(view, y);
	...
	{
		/**
		 * Called whether the header view has been inflated from the resources defined in {@link Options#headerLayout}.
		 * 
		 * @param headerView
		 ...
		 *            - value between 0.0f and 1.0f depending on how far the user has pulled.
		 */
		public abstract void onPulled(float percentagePulled);
		
		/**
		 * Called when a refresh has begun. Theoretically this call is similar to that provided from {@link OnRefreshListener} but is more suitable for header view updates.
		 */
		public abstract void onRefreshStarted();
		
		/**
		 * Called when a refresh can be initiated when the user ends the touch event. This is only called when {@link Options#refreshOnUp} is set to true.
		 */
		public abstract void onReleaseToRefresh();
		
		/**
		 * Called when the current refresh has taken longer than the time specified in {@link Options#refreshMinimizeDelay}.
		 */
	...
	public static abstract class ViewDelegate
	{
		...
		/**
		 * Allows you to provide support for View which do not have built-in support. In this method you should cast <code>view</code> to it's native class, and check if it is scrolled to the top.
		 * 
		 * @param view
	...
	{
		/**
		 * EnvironmentDelegate instance which will be used. If null, we will create an instance of the default class.
		 */
		...
		/**
		 * The header transformer to be used to transfer the header view. If null, an instance of {@link DefaultHeaderTransformer} will be used.
		 */
		...
		/**
		 * The delay after a refresh is started in which the header should be 'minimized'. By default, most of the header is faded out, leaving only the progress bar signifying that a refresh is
		 * taking place.
		 */
	...
	/**
	 * This class allows us to insert a layer in between the system decor view and the actual decor. (e.g. Action Bar views). This is needed so we can receive a call to fitSystemWindows(Rect) so we
	 * can adjust the header view to fit the system windows too.
	 */
	final static class DecorChildLayout extends FrameLayout
	{
		...
		DecorChildLayout(Context context, ViewGroup systemDecorView, View headerView)
		{
			...
			/**
			 * Wrap the Header View in a FrameLayout and add it to this view. It is wrapped so any inset changes do not affect the actual header view.
			 */
